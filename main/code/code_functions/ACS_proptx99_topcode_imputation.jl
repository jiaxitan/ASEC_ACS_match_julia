...
function ACS_PROPTX99_topcode_imputation!(df)
    ...
    df_sample = combine(groupby(df_sample, [:year_group, :statefips]), :txrate => (x -> median(x[(x .< 1) .& (.!ismissing.(x))])) => :txrate_topavg)
    ...
    df[(df.proptx99 .== 69) .& (df.ownershp .== 1), :proptx99_imputed] .= df[(df.proptx99 .== 69) .& (df.ownershp .== 1), :valueh] .* df[(df.proptx99 .== 69) .& (df.ownershp .== 1), :txrate_topavg]
